class Comment
  ...
  def self.create(text:, bookmark_id:)
    result = DatabaseConnection.query("INSERT INTO comments (bookmark_id, text) VALUES ('#{bookmark_id}', '#{text}') RETURNING id, bookmark_id, text;")
    Comment.new(
      id: result[0]['id'],
      text: result[0]['text'],
  ...
  def self.where(bookmark_id:) #.where returns ALL records
